...
public class Field : MonoBehaviour
{
    ...
    {
        var statusTextObject = GameObject.Find("txtStatus");
        if (statusTextObject != null)
        {
            var textComponent = statusTextObject.GetComponent<UnityEngine.UI.Text>();
            if (textComponent != null)
            {
                textComponent.text = "Solve: " + solved + " from " + from;
            }
        }
        else
        {
            Debug.LogWarning("txtStatus tidak ditemukan. Status permainan tidak akan ditampilkan.");
        }
        ...
        if (solved == from) 
        {
            ...
            {
                StartCoroutine(EnableCameraAfterDelay(1f));
            }
        }
    }
